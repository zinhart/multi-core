...
TEST(cpu_test, paralell_transform_cpu)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  //for any needed random uint
  ...
  zinhart::paralell_transform_cpu(x_parallel.get(), x_parallel.get() + n_elements, y_parallel.get(),unary );
...
TEST(cpu_test, paralell_generate_cpu)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  //for any needed random uint
  ...
  zinhart::paralell_generate_cpu(x_parallel.get(), x_parallel.get() + n_elements, generator);

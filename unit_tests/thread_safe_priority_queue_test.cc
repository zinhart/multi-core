...
TEST(thread_safe_priority_queue, call_size_on_empty_queue)
{
  ...
  auto call_size = [](zinhart::multi_core::thread_safe_priority_queue<std::int32_t>  & init_queue)
  {
	ASSERT_EQ(std::uint32_t{0}, init_queue.size());
  };
  zinhart::multi_core::thread_safe_priority_queue<std::int32_t> test_queue;
...
TEST(thread_safe_priority_queue, call_empty_on_empty_queue)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  ...
  std::vector<std::thread> threads(n_threads); 
  auto call_empty = [](zinhart::multi_core::thread_safe_priority_queue<std::int32_t>  & init_queue)
  {
...
TEST(thread_safe_priority_queue, call_clear_on_empty_queue)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<std::uint8_t> thread_dist(1, MAX_CPU_THREADS);
  std::uint8_t n_threads = thread_dist(mt), i;
  std::vector<std::thread> threads(n_threads); 
  auto call_clear = [](zinhart::multi_core::thread_safe_priority_queue<std::int32_t>  & init_queue)
  {
	init_queue.clear();
...
TEST(thread_safe_priority_queue, call_push)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  ...
  auto call_push = [](zinhart::multi_core::thread_safe_priority_queue<std::uint32_t>  & init_queue, std::uint32_t item)
  {
	std::uint32_t old_size = init_queue.size();
	init_queue.push(item);
	ASSERT_TRUE( init_queue.size() >=  old_size);
  };
  zinhart::multi_core::thread_safe_priority_queue<std::uint32_t> test_queue;// sorts largest to smallest
  ...
  for( i = 0; i < n_threads; ++i)
  {
	threads[i] = std::thread(call_push, std::ref(test_queue), i);// push in order from least to greatest
  }
  for(std::thread & t : threads)
  {
	t.join();
  ...
  for( i = 0; i < n_threads; ++i)
  {
	std::uint32_t j{0};
	test_queue.pop(j);
	ASSERT_EQ( n_threads - i - 1, j);
  }
}

TEST(thread_safe_priority_queue, call_size_on_non_empty_queue)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<std::uint32_t> thread_dist(1, MAX_CPU_THREADS);
  ...
  zinhart::multi_core::thread_safe_priority_queue<std::uint32_t> test_queue;
  call_push(test_queue, 0);
  ASSERT_EQ(std::uint32_t{1}, test_queue.size());
  //call push from a random number of threads not exceding MAX_CPU_THREADS
  for( i = 0; i < n_threads; ++i)
...
TEST(thread_safe_priority_queue, call_empty_on_non_empty_queue)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<std::uint8_t> thread_dist(1, MAX_CPU_THREADS);
  ...
  std::vector<std::thread> threads(n_threads); 
  //to be called from each thread
  auto call_push = [](zinhart::multi_core::thread_safe_priority_queue<std::int32_t>  & init_queue, std::int32_t item)
  {
	init_queue.push(item);
...
TEST(thread_safe_priority_queue, call_pop_on_empty_queue)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<std::uint8_t> thread_dist(1, MAX_CPU_THREADS);
  ...
  std::vector<std::thread> threads(n_threads); 
  //to be called from each thread
  auto call_pop = [](zinhart::multi_core::thread_safe_priority_queue<std::int32_t>  & init_queue, std::int32_t item)
  {
...
TEST(thread_safe_priority_queue, call_pop_on_non_empty_queue)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<std::uint8_t> thread_dist(1, MAX_CPU_THREADS);
  ...
  auto test_call_pop = [](zinhart::multi_core::thread_safe_priority_queue<std::int32_t>  & init_queue, std::int32_t & item, bool expected_result)
  {
	bool pop_result = init_queue.pop(item);
...
TEST(thread_safe_priority_queue, call_pop_on_available_on_non_empty_queue)
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<std::uint8_t> thread_dist(1, MAX_CPU_THREADS);
  ...
  std::vector< std::packaged_task<void(zinhart::multi_core::thread_safe_priority_queue<std::int32_t>&, std::int32_t&, bool)> > tasks(n_threads);

  for(i = 0; i < n_threads; ++i)
  {
	// create tasks
	std::packaged_task<void(zinhart::multi_core::thread_safe_priority_queue<std::int32_t>&, std::int32_t&, bool)> test_call_pop(
	[](zinhart::multi_core::thread_safe_priority_queue<std::int32_t>  & init_queue, std::int32_t & item, bool expected_result)
	{
	  init_queue.push(1);

	...
	namespace async
	{
	  ...
	  template< class InputIt, class OutputIt, class UnaryPredicate, class T, class container >
		HOST void replace_copy_if( const InputIt & first, const InputIt & last, OutputIt & output_it, UnaryPredicate unary_predicate, const T& new_value, container & results, thread_pool & default_thread_pool )
		{
		  ...
		  for(thread_id = 0; thread_id < default_thread_pool.size(); ++thread_id)
			results.push_back(
			default_thread_pool.add_task(zinhart::parallel::vectorized::replace_copy_if<InputIt, OutputIt, UnaryPredicate, T>, std::ref(first), std::ref(output_it),std::ref(unary_predicate), std::ref(new_value), thread_id, n_elements, default_thread_pool.size() )
			);
		}
	  ...
	  template < class InputIt, class OutputIt, class UnaryOperation, class container >
		HOST void transform(const InputIt & first, const InputIt & last, OutputIt & output_first, UnaryOperation unary_op, container & results, thread_pool & default_thread_pool )
		{
		  ...
		  for(thread_id = 0; thread_id < default_thread_pool.size(); ++thread_id)
			results.push_back(
			default_thread_pool.add_task(zinhart::parallel::vectorized::transform<InputIt, OutputIt, UnaryOperation>,  std::ref(first), std::ref(output_first), std::ref(unary_op), thread_id, n_elements, default_thread_pool.size() )
			);
		}
	  template < class BidirectionalIt, class Generator, class container >
		HOST void generate(const BidirectionalIt & first, const BidirectionalIt & last, Generator g, container & results, thread_pool & default_thread_pool)
		{
		  ...
		  for(thread_id = 0; thread_id < default_thread_pool.size(); ++thread_id)
			results.push_back(
			default_thread_pool.add_task(zinhart::parallel::vectorized::generate<BidirectionalIt, Generator>, std::ref(first), std::ref(g), thread_id, n_elements, default_thread_pool.size())
			);
		}
	  ...
	  template <class precision_type, class container>
						   ...
						   )
		{
		  ...
		  for(thread_id = 0; thread_id < default_thread_pool.size(); ++thread_id)
			results.push_back(
			default_thread_pool.add_task(zinhart::parallel::vectorized::kahan_sum<precision_type, binary_predicate>, vec_1, vec_2, std::ref(global_sum), bp, thread_id, data_size, default_thread_pool.size())
			);
		}
	  ...
	  template <class precision_type, class container, class binary_predicate>
							  ...
							  )
		{
		  ...
		  for(thread_id = 0; thread_id < default_thread_pool.size(); ++thread_id)
			results.push_back(
			default_thread_pool.add_task(zinhart::parallel::vectorized::neumaier_sum<precision_type, binary_predicate>, vec_1, vec_2, std::ref(global_sum), bp, thread_id, data_size, default_thread_pool.size())
			);
		}
	}// END NAMESPACE ASYNC

  ...
  namespace multi_core
  {
	...
	namespace async
	{
	  template <class precision_type> 
		HOST void saxpy(const precision_type & a, precision_type * x, precision_type * y, const std::uint32_t n_elements, const std::uint32_t n_threads = 1, const std::uint32_t thread_id = 0 );
	  /*
	  template<class InputIt, class OutputIt, class container>
		HOST void copy(const InputIt & first, const InputIt & last, OutputIt & output_it, container & results, 
			                        thread_pool::pool & thread_pool = zinhart::multi_core::thread_pool::get_thread_pool()
								   );
